...
# ${SHORTCUT}
alias dl='script() { COMMAND="docker logs -f "\$1""; echo "当前账号是：\$(whoami)"; \$(if getent group docker | grep -q "\b\$USER\b"; then eval "\$COMMAND"; else echo "正在升级成ROOT账号，请输入密码"; eval "sudo \$COMMAND"; fi); }; script'
EOF
fi
