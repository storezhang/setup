...
# ${SHORTCUT}
alias dl='script() { COMMAND="docker logs -f "\$1""; echo "当前账号是：\$(whoami)"; getent group docker | grep -q "\b\$USER\b" && echo "用户已在Docker组内，不需要ROOT权限，继续执行"; eval "\$COMMAND" || echo "正在升级成ROOT账号，请输入密码"; eval "sudo \$COMMAND"; }; script'
EOF
fi
...
# ${SHORTCUT}
alias di='script() { COMMAND="docker exec --interactive --tty "\$1" /bin/bash"; echo "当前账号是：\$(whoami)"; getent group docker | grep -q "\b\$USER\b" && echo "用户已在Docker组内，不需要ROOT权限，继续执行"; eval "\$COMMAND" || echo "正在升级成ROOT账号，请输入密码"; eval "sudo \$COMMAND"; }; script'
EOF
fi
...
# ${SHORTCUT}
alias drs='script() { COMMAND="docker run --interactive --tty --rm --entrypoint /bin/"\$1" "\$2""; echo "当前账号是：\$(whoami)"; getent group docker | grep -q "\b\$USER\b" && echo "用户已在Docker组内，不需要ROOT权限，继续执行"; eval "\$COMMAND" || echo "正在升级成ROOT账号，请输入密码"; eval "sudo \$COMMAND"; }; script'
EOF
fi
